// PASO 5F-R — Piloto de indicadores para anillos de periferia externa.
// Revisión del 28-07-2026. No incluye el enclave interno de Carabayllo 2.
// Analiza cinco años y ambos niveles espaciales. No exporta archivos.
...
var piloto = ee.FeatureCollection(ee.List(nestedRows).flatten());
var pilotoAmbitos = piloto.filter(ee.Filter.eq('nivel', 'ambito'));
var pilotoSistema = piloto.filter(ee.Filter.eq('nivel', 'sistema'));

print('PASO 5F-R — PILOTO DE ANILLOS CORREGIDOS');
print('Años piloto', years);
print('Bandas métricas — esperado: 20', metricsStack.bandNames().size());
print('Unidades espaciales — esperado: 18', wide.size());
print('Filas totales — esperado: 90', piloto.size());
print('Filas por ámbito — esperado: 75', pilotoAmbitos.size());
print('Filas del sistema — esperado: 15', pilotoSistema.size());
print(
  'Unidades únicas — esperado: 18',
  piloto.aggregate_array('unidad_id').distinct().size()
);
print(
  'Años únicos — esperado: 5',
  piloto.aggregate_array('year').distinct().sort()
);
print(
  'Sin dato máximo — porcentaje',
  piloto.aggregate_max('sin_dato_pct')
);
print(
  'Diferencia píxeles frente a área UTM — máxima, %',
  piloto.aggregate_max('dif_pix_utm_abs_pct')
);
print('Vista previa — anillos por ámbito', pilotoAmbitos.limit(10));
print('Vista previa — sistema disuelto', pilotoSistema.limit(10));
...
print('PASO 5F — PILOTO FINALIZADO; no se realizaron exportaciones');
