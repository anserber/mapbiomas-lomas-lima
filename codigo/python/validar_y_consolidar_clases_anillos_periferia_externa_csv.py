#!/usr/bin/env python3
"""Consolida y audita los ocho CSV corregidos de la periferia externa."""
...
INDICATORS = (
    RAW / "serie_indicadores_anillos_periferia_externa_1985_2024.csv"
)
OUT_CSV = (
    RESULTS / "serie_clases_anillos_periferia_externa_1985_2024.csv"
)
REPORT = (
    RESULTS
    / "validacion_serie_clases_anillos_periferia_externa_1985_2024.md"
)
OUT_SHA = (
    RESULTS / "serie_clases_anillos_periferia_externa_1985_2024.csv.sha256"
)
INPUT_MANIFEST = (
    RAW / "manifest_sha256_serie_clases_anillos_periferia_externa.txt"
)
...
def main() -> None:
    ...
    for start, end, expected_count in BLOCKS:
        path = (
            RAW
            / f"serie_clases_anillos_periferia_externa_{start}_{end}.csv"
        )
    ...
    report = [
        "# Validación de clases de la periferia externa 1985–2024",
        "",
        f"**Estado:** {status}  ",
    ...
    if status == "PASS":
        report.append(
            "La composición completa por clases de la periferia externa es apta "
            "para el análisis. El enclave interno de SEDAPAL de Carabayllo 2 está "
            "excluido de estos anillos y se conserva como unidad espacial separada. "
            "La clase 0 representa explícitamente píxeles sin dato y no debe "
            "combinarse con las clases de cobertura. Para totales del sistema deben "
            "utilizarse únicamente las tres unidades con nivel `sistema`."
        )
    else:
